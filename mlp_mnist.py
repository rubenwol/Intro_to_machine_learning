...
class KerasMnist(object):
    ...
    def build_model_no_skip(self):
        ...
        for i in range(1,len(self.hidden_layer_dims)-1):
        ...
        self.model.compile(loss='categorical_cross_entropy',
                           optimizer=SGD(),
                           metrics=['accuracy'])
        self.model.summary()
    ...
    def build_model_skip(self):
        ...
        k = x
        cnt = 0
        for i in self.hidden_layer_dims:
            if (cnt != 0):
                if (cnt + 1) % self.skips == 0:
                    k = keras.layers.add([k, skip])
                    skip = k
                k = Dense(i, activation='relu')(k)
            else:
                k = Dense(i, activation='relu')(k)
                skip = k
            cnt += 1
        out = Dense(self.num_classes, activation='softmax')(k)
        ### YOUR CODE ENDS HERE
